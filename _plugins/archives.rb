  ...
  class ArchiveGenerator < Generator
    ...
    def generate(site)
      if site.layouts.key? 'archive_index'
        ...
        site.posts.group_by{ |c| {"day" => c.date.day, "month" => c.date.month, "year" => c.date.year} }.each do |period, posts|
          archive_dir = File.join(period["year"].to_s(), "%02d" % period["month"].to_s(), "%02d" % period["day"].to_s())
          write_archive_index(site, archive_dir, period, posts)
        end        
      end
    end
